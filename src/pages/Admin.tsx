...
export default function Admin() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (setData.rulesPdfOriginalName) {
        setRulesPdfOriginalName(setData.rulesPdfOriginalName);
      }
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      await fetch('/api/admin/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: 'rulesPdfUrl', value: rulesPdfUrl })
      });
      await fetch('/api/admin/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
                        {sponsors.map((url, idx) => (
                          <div key={idx} className="relative group bg-[#111] border-2 border-[#333] p-4">
                            <img src={url} alt={`Sponsor ${idx}`} className="h-32 w-full object-contain" />
                            <button
